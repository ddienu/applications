document.addEventListener('DOMContentLoaded', async ()=> {
  ...
  await checkAuth();
  console.log('users api controller has been loaded');
  fadeInElement(document.querySelector('body'), 1000);
  // Initialize the loading screen
});


const objForm = new Form('usersApiForm', 'edit-input');
...
myForm.addEventListener('submit', (e) => {
  ...
  if (insertUpdate) {
    ...
    endpointUrl = URL_USERS_API;
  } else {
    console.log("Update");
    httpMethod = METHODS[2]; // PUT method
    endpointUrl = URL_USERS_API + keyId;
  }
  ...
  resultServices.then(response => {
    ...
    // console.log("data en linea 50 usersApiController", data);
  }).catch(error => {
    console.log(error);
  }).finally(() => {
    //console.log("finally");
...
function getData() {
  ...
  resultServices.then(response => {
    ...
    //Create table 
    // console.log("data en la linea 138 del usersApiController", data);
    createTable(data);
  }).catch(error => {
    console.log(error);
  }).finally(() => {
